...
OUTPUT = RUN_ROOT / "outputs" / "02_ukpn_stage_weather_working_v3.csv"

# Reuse the proven root-level cache without copying or changing its contents.
UNIQUE_REQUEST_CACHE_DIR = PROJECT_ROOT / "data" / "weather_request_cache"
FAILED_REQUEST_LOG = RUN_ROOT / "logs" / "02_weather_failed_requests.csv"
MISSING_INCIDENT_LOG = RUN_ROOT / "logs" / "02_weather_missing_rows.csv"
CHECKPOINT_PATH = RUN_ROOT / "logs" / "02_weather_checkpoint.json"

REQUESTS_CACHE_PATH = PROJECT_ROOT / ".weather_cache_master"
...
def append_rows_to_csv(df_append, csv_path):
    ...
    if not header:
        ...
        new_columns = [c for c in df_append.columns if c not in existing_columns]
        if new_columns:
            raise ValueError(f"Incremental output schema changed unexpectedly: {new_columns}")
        df_append = df_append.reindex(columns=existing_columns)
...
def process_incidents_and_flush(
    ...
    for i, (_, row) in enumerate(
        ...
        feature_key = (request_key, pd.Timestamp(row["clean_start"]).floor("h"))
        feat = feature_cache.get(feature_key)
        if feat is None:
            feat = build_incident_features_from_hourly(
                df_hourly=df_hourly,
                target_time_utc=row["clean_start"]
            )
            feature_cache[feature_key] = feat

        combined = {
            **row.to_dict(),
            **feat.to_dict(),
            "weather_status_v3": "matched"
        }
...
def append_rows_without_weather(rows_df, processed_ids, output_path, reason):
    """Append source rows with an explicit weather status and empty weather fields."""
    if rows_df.empty:
        return 0
    empty_features = make_empty_feature_series().to_dict()
    weather_block = pd.DataFrame(
        {col: np.full(len(rows_df), value) for col, value in empty_features.items()},
        index=rows_df.index,
    )
    output = pd.concat([rows_df.copy(), weather_block], axis=1)
    output["weather_status_v3"] = reason
    append_rows_to_csv(output, output_path)
    processed_ids.update(output["source_row_number"].astype(str).tolist())
    return len(output)
...
def run_power_outage_weather_pipeline(
    ...
    required_cols = [TIME_COL, COORD_COL, "Incident Reference", ID_COL]
    ...
    df["request_key"] = request_key.where(df["weather_eligible_v3"], pd.NA)
    ...
    invalid_rows = df_to_process[~df_to_process["weather_eligible_v3"]].copy()
    invalid_written = append_rows_without_weather(
        invalid_rows,
        processed_ids,
        output_path,
        reason="invalid_time_or_coordinate",
    )
    if invalid_written:
        print(f"已保留但无法匹配天气的原始行：{invalid_written}")
    df_to_process = df_to_process[df_to_process["weather_eligible_v3"]].copy()
    ...
    unavailable = df_to_process[
        ~df_to_process["request_key"].isin(weather_request_store.keys())
        & ~df_to_process[ID_COL].isin(processed_ids)
    ].copy()
    unavailable_written = append_rows_without_weather(
        unavailable,
        processed_ids,
        output_path,
        reason="weather_request_unavailable",
    )
    ...
    write_json({
        ...
        "written_without_weather_this_run": unavailable_written
    }, checkpoint_path)
...
if __name__ == "__main__":
    TEST_MODE = os.environ.get("V3_TEST_MODE", "0") == "1"
    TEST_N = int(os.environ.get("V3_TEST_N", "20"))
    FETCH_MISSING = os.environ.get("V3_FETCH_MISSING", "0") == "1"
    ...
    run_power_outage_weather_pipeline(
        ...
        coord_round_decimals=1,
        debug=True,
        save_every=20 if TEST_MODE else 2000,
        sleep_sec_formal=0.25,
