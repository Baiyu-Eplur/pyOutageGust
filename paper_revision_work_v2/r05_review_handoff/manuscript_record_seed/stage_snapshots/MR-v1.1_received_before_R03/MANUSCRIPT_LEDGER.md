...
版本：MR-v1.1，2026-09-05。检查点：**初步代码核查 + R00–R01 + R02返回摘要与完整报告已接收；下一步R03**。用户所说的“S0-1”在本次记录中对应v2计划的R00–R01；不另造一次审计。

## 0. 使用范围与当前事实

这是一份已经填入具体内容的论文修订记录。它同时保存研究主线、逐章方案、主张与证据对应、具体修改条目、来源、状态和下一步。后续每阶段都需更新，而不是只保存代码报告或依靠对话记忆。

当前新增接收SRC09/SRC10：R02_input_20260905摘要与完整报告。**R02代理规则、输入重建、实际入口和针对性检查据本地报告已完成；本环境未独立读取全部contracts/源码/主表/缓存及测试日志或重跑研究。报告可支持进入R03，不等于B1或科学验证完成。R03/R04/V尚未执行，Word未修改。** 两报告为同次运行，不能算两次独立验证。
...
|---|---|---|---|---|
| CL01 | 重构指标是否测到所声称的事件客户影响与恢复跨度 | 字段语义、身份规则、完整阶段、C/D实现、边界及新样本流 | R02输入/全阶段聚合/实际入口据报告通过；真实onset/特殊规则仍有边界 | 使用记录跨度/时间代理，明确未决资格；不写平均客户失电或CML |
| CL02 | 阵风关系是否存在可重复的非线性/最低点 | B1后形状比较、支持量、影响过程、必要区间 | R02已改变输入，旧形状与三阶线索仍是H0，尚未重估 | 非线性不稳则报告不稳或简单模型；最低点不稳则撤出摘要 |
| CL03 | 阵风与最终客户规模的相对增量信息是否随原因人群改变 | 同信息条件/样本/分母的配对Δ、统一尾部/划分、共同支持及不确定性 | R02候选已明确；尾部/折分接口及新配对结果尚无 | 贡献接近且点估计反转；或没有稳定反转，均允许 |
| CL04 | 完整天气过程中的关系与误差如何 | 过程/连续块评价、窗口合并、成员和权重、基准误差 | R02已用UNIQUE_ANY，原图预测身份尚未修，仍非独立过程验证 | 仅作描述；若泛化差，报告局限而非把相关性包装为验证 |
