# A_VARIABLE_DICTIONARY

需求 A02；A.2。变量、单位、来源、变换、参考水平。
...
正文使用变换后OLS；不是GLM log link。地区变量定义复用旧附录A，不迁移旧同字母章节的结论。

| variable | unit | source | transform_definition | reference_scope |
| --- | --- | --- | --- | --- |
| customers_v2_event_excl_reinterruptions | customers | 监管阶段记录 | 非再中断阶段客户数求和；E0响应ln(1+C) | 零保留于E0，R0最终排除 |
| duration_B_full_span_hours | h | 监管阶段起止 | 全部阶段最迟结束减最早开始；R0响应ln(D) | 恢复输入已有D>0和上端截断，最终再排C=0 |
| duration_A_customer_weighted_hours | h | 监管阶段客户/时长 | 客户加权时长；只作构造比较 | 不作为最终恢复响应 |
