...
F01/F03原整理时间：2026-09-10T13:17:06+01:00。本文件是中文产物整理说明，不是正式附录。

复现：`python main_appendix.py --appendices F`；项目根目录以脚本位置解析。

四组最终双向完整系数已具备；天气final原表已有se_lad；本包补齐矩阵、区间与可复算来源。联合地区+日历块不可拆成日历因果作用。

## 关键结果摘要

完整系数共168行（含全体LAD-only与双向SE，以及天气双向SE）。四组结果均保留，不按p值筛选。

## 需求、产物与适用范围

### F01 — 四组完整双向聚类系数和全体LAD-only对照

已生成。正文定位：§3.4 P040; §4.1 P052–053; §4.2 P061。G为LAD数；最终公式为对数结果OLS；参考水平和标准化见A02。

- [tables/F_FULL_COEFFICIENTS.csv](tables/F_FULL_COEFFICIENTS.csv)

- [tables/F_FULL_COEFFICIENTS.md](tables/F_FULL_COEFFICIENTS.md)

### F03 — 四组完整顺序增量结果

已生成。正文定位：§4.3 P064; Table 6。Regional + calendar是联合块，不能写成calendar单独贡献；顺序分解不是唯一因果分解。

- [tables/F03_R2_DECOMPOSITION.csv](tables/F03_R2_DECOMPOSITION.csv)

- [tables/F03_R2_DECOMPOSITION.md](tables/F03_R2_DECOMPOSITION.md)
