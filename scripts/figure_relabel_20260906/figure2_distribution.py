"""Command #52: relabeled copy of figure2_distribution.py. ONLY change:
panel (a)'s x-axis label no longer includes the internal codename
"customers_v2" -- now reads "Affected customers" like the other three
panels' formal labels. No data, styling, or statistical content changed."""
...
def log_step(msg):
    print(f"[fig2-relabel] {msg}", flush=True)
...
def main():
    ...
    # RELABEL: "Affected customers (customers_v2)" -> "Affected customers"
    hist_with_kde(axes[0, 0], customers[customers <= p99_c], COLOR_CUST, bins=60, xlabel="Affected customers")
