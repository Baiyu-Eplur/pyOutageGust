"""Command #52: relabeled copy of figure8_weather_subsample_comparison.py.
Changes: "weather_natural" x-tick labels -> "Weather-related"; "E0 (exposure)"
text annotation -> "Exposure margin"; "R0c (recovery)" text annotation ->
"Recovery margin". No data or statistics changed."""
...
def log_step(msg):
    print(f"[fig8-relabel] {msg}", flush=True)


def main():
    apply_style()
    e0_main = pd.read_csv(read_input(C01_RAW / "step4b_E0_corrected_variance_decomposition.csv"))
    e0_wn = pd.read_csv(read_input(RAW_DIR / "E0_wn_corrected_variance_decomposition.csv"))
    r0c_main = pd.read_csv(read_input(C01_RAW / "step4b_R0c_corrected_variance_decomposition_gust_first.csv"))
    r0c_wn = pd.read_csv(read_input(RAW_DIR / "R0c_wn_corrected_variance_decomposition_gust_first.csv"))
    ...
    ax = axes[0]
    # RELABEL: "weather_natural" -> "Weather-related"
    ...
    ax.text(0.95, 0.95, "Exposure margin", transform=ax.transAxes, fontsize=9, va="top", ha="right", color="#444444")
    panel_label(ax, "(a)")
    ...
    # RELABEL: "R0c (recovery)" -> "Recovery margin"
    ax.text(0.95, 0.95, "Recovery margin", transform=ax.transAxes, fontsize=9, va="top", ha="right", color="#444444")
    panel_label(ax, "(b)")
    ...
    plt.close(fig)
    log_step("Saved Figure_8_weather_subsample_comparison (relabeled).")
