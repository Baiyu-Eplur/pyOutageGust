"""Command #47: A02 resolution -- Figure 6a/6b redrawn with the choropleth
color scale showing the model's log-scale predicted value directly (no
exponentiation). Identical to figure6_regional_map.py's command #45 pipeline
except `pred = eta` replaces `pred = np.exp(eta)`, and the colorbar range is
independently re-derived from the new log-scale values (not forced to match
the old raw-scale range) with an explicit log-scale legend label.
"""
...
def log_step(msg):
    print(f"[fig6-logscale] {msg}", flush=True)
...
def fit_and_predict_by_lad(v9, sample, target_col, use_customers, label):
    ...
    # Command #47: no exponentiation -- the choropleth shows log-scale eta directly.
    pred_log = eta
    return pd.DataFrame({"LAD21CD": lads, f"pred_log_{label}": pred_log.values})
...
def main():
    ...
    log_step("Predicting baseline regional log(1+customers_v2) (E0, corrected)...")
    pred_customers = fit_and_predict_by_lad(v9, combined_e0, "log1p_customers_v2", False, "customers_v2")
    log_step("Predicting baseline regional log(duration_B) (R0c, corrected, customers fixed at z=0)...")
    ...
    pred_lad.to_csv(RAW_DIR / "figure6_regional_predictions_log_scale.csv", index=False)

    corr_pearson = pred_lad[["pred_log_customers_v2", "pred_log_duration_B"]].corr().iloc[0, 1]
    corr_spearman = pred_lad[["pred_log_customers_v2", "pred_log_duration_B"]].corr(method="spearman").iloc[0, 1]
    log_step(f"Predicted-surface correlation (log scale): Pearson={corr_pearson:.3f}, Spearman={corr_spearman:.3f}")
    log_step("(For comparison against command #45's Figure 6: the Spearman rank correlation, and the "
    ...
    def plot_map(value_col, out_stub, title_note, cbar_label):
        ...
        map_gdf.plot(column=value_col, cmap="Oranges", linewidth=0.18, edgecolor="white",
                      ...
                      legend_kwds={"shrink": 0.6, "label": cbar_label})
        ...
        plt.close(fig)
        log_step(f"Saved {out_stub} (vmin={vmin:.4f}, vmax={vmax:.4f}) [{title_note}]")

    plot_map("pred_log_customers_v2", "Figure_6a_regional_customers_log_scale",
              "predicted log(1+affected customers)", "log(1 + affected customers)")
    plot_map("pred_log_duration_B", "Figure_6b_regional_duration_log_scale",
              "predicted log(restoration duration)", "log(restoration duration)")
    log_step("Done.")
